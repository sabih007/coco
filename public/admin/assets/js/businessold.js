...
$('#cancelModal').on('show.bs.modal', function (event) {
    var button = $(event.relatedTarget)
    var cancel_reason = button.data('cancel_reason')
    var modal = $(this)
    modal.find('.modal-body #BookingID').val(bookingid)
    modal.find('.modal-body #cancel_reason').val(cancel_reason)
})

$("#cancelbooking").submit(function(){
    cancel_reason = $("#cancel_reason").val();
    url = $(this).attr("action");
    $.post(url,{'CancellationReason':cancel_reason},function(fb){
        if (fb.match('1')) {
