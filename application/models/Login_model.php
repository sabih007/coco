...
class Login_model extends CI_Model
{
    ...
    function loginMe($email, $password)
    {
     ...
     function loginMe_admin($email, $password, $role)
    {
       $this->db->select('BaseTbl.userId, BaseTbl.password, BaseTbl.name, BaseTbl.roleId, Roles.role, BaseTbl.email');
        $this->db->from('tbl_users as BaseTbl');
        $this->db->join('tbl_roles as Roles','Roles.roleId = BaseTbl.roleId');
    ...
    function updateTableHOf($data, $insert_id){
        $data = array(
            'FirstName' => $data['name'],
            'LastName'=>$data['lastname'],
            'ContactNo1' => $data['mobile'],
            'Email' => $data['email'],
            'login_id' => $insert_id
        );
    ...
    function fb_post(){
        return $this->db->select('*')->from('fb_post')->get()->result_array();
    }
